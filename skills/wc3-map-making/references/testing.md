...
- **The app's own launch does.** The app starts the game as `-launch -uid w3` and hands it its session, so `game_test login="auto"` (the default) asks the app to launch the game: no login panel at all. The map is copied to one fixed path in the server's folder and that path goes into the app's launch options for Warcraft III (`Battle.net.config`, `Games.w3.AdditionalLaunchArguments`), which the app reads when it starts - so the first run that stores them restarts the app (`login.battlenet.restarted`), later runs do not. `launched_by` says which route a run took, and the copy is deleted when the run closes the game.
