...
- **Water crossings are not proven until the editor has saved the map.** A crossing the derived pathing read as 320 units wide over a `river` bed came back closed in the editor's own `war3map.wpm`, and natural bridges (`YT04`) added no walkable ground in `map_flow`. Carve the crossing, save it in the editor (`editor_map open` + `save`), then check it with `map_flow`; `map_flow` carries a `warning` while the pathing it used is derived.
